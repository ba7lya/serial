...
| 2_1  | is_open     | 打开未占用的串口，验证open()返回值是否与is_open()相等         |           |         |
| 2_2  | is_open     | 打开占用的串口，验证open()返回值是否与is_open()相等           |           |         |
...
| 4_1  | open       | 同步方式打开两个串口, 串口1写入数据，串口2读取数据    |           |         |
| 5_1  | open       | 异步方式打开两个串口, 串口1写入数据，串口2读取数据    |           |         |